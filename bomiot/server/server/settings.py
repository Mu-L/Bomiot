...
# Load _DISCOVERED_APPS from WORKING_SPACE/apps.json (generated by `bomiot run` pre-step).
# WORKING_SPACE already aligns with IS_LAN (exe dir / os.getcwd()) at lines 22-25 above.
_DISCOVERED_APPS = []
try:
    _apps_json_path = join(WORKING_SPACE, 'apps.json')
    if exists(_apps_json_path):
        try:
            import orjson as _orjson
            _data = _orjson.loads(Path(_apps_json_path).read_bytes())
        except Exception:
            import json as _json
            with open(_apps_json_path, 'r', encoding='utf-8') as _f:
                _data = _json.load(_f)
        if isinstance(_data, list):
            _DISCOVERED_APPS = [str(x) for x in _data if isinstance(x, str)]
except Exception:
    logging.getLogger(__name__).debug("Failed to load apps.json from %s", WORKING_SPACE)
...
JWT_SALT = secrets.token_urlsafe(32)
